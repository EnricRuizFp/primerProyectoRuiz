<footer class="container-fluid">
    <div class="row">
        <div id="seccionIzquierdaFooter" class="col-12 col-xl-9 container-fluid">

            <div id="superiorIzquierdaFooter" class="row container-fluid">
                <div class="col-12 col-sm-6 col-lg-3 row">
                    <div class="tituloColumnaSuperiorSeccionIzquierdaFooter col-12">
                        <a href="?controller=general&action=productos"><h7 class="naranja">PRODUCTOS</h7></a>
                    </div>
                    <div class="subtitulosColumnaSuperiorSeccionIzquierdaFooter col-12">
                        <a href="?controller=general&action=ofertas"><p class="p4 blanco">Ofertas</p></a>
                    </div>
                    <div class="subtitulosColumnaSuperiorSeccionIzquierdaFooter col-12">
                        <a href="?controller=general&action=pizzas"><p class="p4 blanco">Pizzas</p></a>
                    </div>
                    <div class="subtitulosColumnaSuperiorSeccionIzquierdaFooter col-12">
                        <a href="?controller=general&action=bebidas"><p class="p4 blanco">Bebidas</p></a>
                    </div>
                    <div class="subtitulosColumnaSuperiorSeccionIzquierdaFooter col-12">
                        <a href="?controller=general&action=postres"><p class="p4 blanco">Postres</p></a>
                    </div>
                </div>

                <div class="col-12 col-sm-6 col-lg-3 row">
                    <div class="tituloColumnaSuperiorSeccionIzquierdaFooter col-12">
                        <a href="?controller=general&action=nutricionYcalidad"><h7 class="naranja">NUTRICIÓN Y CALIDAD</h7></a>
                    </div>
                    <div class="subtitulosColumnaSuperiorSeccionIzquierdaFooter col-12">
                        <a href="?controller=general&action=nutricionYcalidad#calidad"><p class="p4 blanco">Calidad y alimentación</p></a>
                    </div>
                    <div class="subtitulosColumnaSuperiorSeccionIzquierdaFooter col-12">
                        <a href="?controller=general&action=nutricionYcalidad#masas"><p class="p4 blanco">Nuestras masas y alimentos</p></a>
                    </div>
                    <div class="subtitulosColumnaSuperiorSeccionIzquierdaFooter col-12">
                        <a href="?controller=general&action=nutricionYcalidad#sinGluten"><p class="p4 blanco">Pizzas sin gluten</p></a>
                    </div>
                </div>

                <div class="col-12 col-sm-6 col-lg-3 row">
                    <div class="tituloColumnaSuperiorSeccionIzquierdaFooter col-12">
                        <a href="?controller=general&action=corporativo"><h7 class="naranja">CORPORATIVO</h7></a>
                    </div>
                    <div class="subtitulosColumnaSuperiorSeccionIzquierdaFooter col-12">
                        <a href="?controller=general&action=corporativo#acercaDeLaBuenizzima"><p class="p4 blanco">Acerca de La Buenizzima</p></a>
                    </div>
                    <div class="subtitulosColumnaSuperiorSeccionIzquierdaFooter col-12">
                        <a href="?controller=general&action=corporativo#trabajaConNosotros"><p class="p4 blanco">Trabaja con nosotros</p></a>
                    </div>
                </div>

                <div class="col-12 col-sm-6 col-lg-3 row">
                    <div class="tituloColumnaSuperiorSeccionIzquierdaFooter col-12">
                        <a href="?controller=general&action=avisoLegal"><h7 class="naranja">AVISO LEGAL</h7></a>
                    </div>
                    <div class="subtitulosColumnaSuperiorSeccionIzquierdaFooter col-12">
                        <a href="?controller=general&action=avisoLegal#politicaDePrivacidad"><p class="p4 blanco">Política de privacidad</p></a>
                    </div>
                    <div class="subtitulosColumnaSuperiorSeccionIzquierdaFooter col-12">
                        <a href="?controller=general&action=avisoLegal#politicaDeCookies"><p class="p4 blanco">Política de cookies</p></a>
                    </div>
                </div>
            </div>

            <div id="centralIzquierdaFooter">
                <hr>
            </div>

            <div id="inferiorIzquierdaFooter" class="row">
                <div class="col-6">
                    <p>©️ Copyright La buenizzima 2024</p>
                </div>
                <div class="col-6 alignRight">
                    <p>All rights reserved.</p>
                </div>
            </div>
        </div>

        <div id="seccionDerechaFooter" class="col-12 col-xl-3 container-fluid">
            <div id="superiorSeccionDerechaFooter" class="row container-fluid">
                <h7 class="naranja bold text-center">SÍGUENOS</h7>
                <div id="contenedorRedesSociales" class="row">
                    <div class="col-2 offset-2">
                        <a href="https://www.instagram.com/">
                            <!-- ICONO IG -->
                            <svg viewBox="0 0 20 20" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000" width="30px">
                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                <g id="SVGRepo_iconCarrier">
                                    <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                        <g id="Dribbble-Light-Preview" transform="translate(-340.000000, -7439.000000)" fill="#FFFFFF">
                                            <g id="icons" transform="translate(56.000000, 160.000000)">
                                                <path d="M289.869652,7279.12273 C288.241769,7279.19618 286.830805,7279.5942 285.691486,7280.72871 C284.548187,7281.86918 284.155147,7283.28558 284.081514,7284.89653 C284.035742,7285.90201 283.768077,7293.49818 284.544207,7295.49028 C285.067597,7296.83422 286.098457,7297.86749 287.454694,7298.39256 C288.087538,7298.63872 288.809936,7298.80547 289.869652,7298.85411 C298.730467,7299.25511 302.015089,7299.03674 303.400182,7295.49028 C303.645956,7294.859 303.815113,7294.1374 303.86188,7293.08031 C304.26686,7284.19677 303.796207,7282.27117 302.251908,7280.72871 C301.027016,7279.50685 299.5862,7278.67508 289.869652,7279.12273 M289.951245,7297.06748 C288.981083,7297.0238 288.454707,7296.86201 288.103459,7296.72603 C287.219865,7296.3826 286.556174,7295.72155 286.214876,7294.84312 C285.623823,7293.32944 285.819846,7286.14023 285.872583,7284.97693 C285.924325,7283.83745 286.155174,7282.79624 286.959165,7281.99226 C287.954203,7280.99968 289.239792,7280.51332 297.993144,7280.90837 C299.135448,7280.95998 300.179243,7281.19026 300.985224,7281.99226 C301.980262,7282.98483 302.473801,7284.28014 302.071806,7292.99991 C302.028024,7293.96767 301.865833,7294.49274 301.729513,7294.84312 C300.829003,7297.15085 298.757333,7297.47145 289.951245,7297.06748 M298.089663,7283.68956 C298.089663,7284.34665 298.623998,7284.88065 299.283709,7284.88065 C299.943419,7284.88065 300.47875,7284.34665 300.47875,7283.68956 C300.47875,7283.03248 299.943419,7282.49847 299.283709,7282.49847 C298.623998,7282.49847 298.089663,7283.03248 298.089663,7283.68956 M288.862673,7288.98792 C288.862673,7291.80286 291.150266,7294.08479 293.972194,7294.08479 C296.794123,7294.08479 299.081716,7291.80286 299.081716,7288.98792 C299.081716,7286.17298 296.794123,7283.89205 293.972194,7283.89205 C291.150266,7283.89205 288.862673,7286.17298 288.862673,7288.98792 M290.655732,7288.98792 C290.655732,7287.16159 292.140329,7285.67967 293.972194,7285.67967 C295.80406,7285.67967 297.288657,7287.16159 297.288657,7288.98792 C297.288657,7290.81525 295.80406,7292.29716 293.972194,7292.29716 C292.140329,7292.29716 290.655732,7290.81525 290.655732,7288.98792" id="instagram-[#167]"> </path>
                                            </g>
                                        </g>
                                    </g>
                                </g>
                            </svg>
                        </a>
                    </div>
                    <div class="col-2">
                        <a href="https://x.com/?lang=ca">
                            <!-- ICONO X -->
                            <svg xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision" text-rendering="geometricPrecision" image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd" viewBox="0 0 512 509.64" width="30px">
                                <rect width="512" height="509.64" rx="115.61" ry="115.61" fill="#fff"/> <!-- Fondo cambiado a blanco -->
                                <path fill="#000" fill-rule="nonzero" d="M323.74 148.35h36.12l-78.91 90.2 92.83 122.73h-72.69l-56.93-74.43-65.15 74.43h-36.14l84.4-96.47-89.05-116.46h74.53l51.46 68.04 59.53-68.04zm-12.68 191.31h20.02l-129.2-170.82H180.4l130.66 170.82z"/>
                            </svg>
                        </a>
                    </div>
                    <div class="col-2">
                        <a href="https://www.facebook.com/">
                            <!-- ICONO FACEBOOK -->
                            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30px" viewBox="0,0,256,256">
                                <g fill="#ffffff" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt" stroke-linejoin="miter" stroke-miterlimit="10" stroke-dasharray="" stroke-dashoffset="0" font-family="none" font-weight="none" font-size="none" text-anchor="none" style="mix-blend-mode: normal">
                                    <g transform="scale(5.12,5.12)">
                                        <path d="M25,3c-12.15,0 -22,9.85 -22,22c0,11.03 8.125,20.137 18.712,21.728v-15.897h-5.443v-5.783h5.443v-3.848c0,-6.371 3.104,-9.168 8.399,-9.168c2.536,0 3.877,0.188 4.512,0.274v5.048h-3.612c-2.248,0 -3.033,2.131 -3.033,4.533v3.161h6.588l-0.894,5.783h-5.694v15.944c10.738,-1.457 19.022,-10.638 19.022,-21.775c0,-12.15 -9.85,-22 -22,-22z"></path>
                                    </g>
                                </g>
                            </svg>
                        </a>
                    </div>
                    <div class="col-2">
                        <a href="https://www.youtube.com/">
                            <!-- ICONO YT -->
                            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30px" viewBox="0,0,256,256">
                                <g fill="#ffffff" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt" stroke-linejoin="miter" stroke-miterlimit="10" stroke-dasharray="" stroke-dashoffset="0" font-family="none" font-weight="none" font-size="none" text-anchor="none" style="mix-blend-mode: normal">
                                    <g transform="scale(5.12,5.12)">
                                        <path d="M44.89844,14.5c-0.39844,-2.19922 -2.29687,-3.80078 -4.5,-4.30078c-3.29687,-0.69922 -9.39844,-1.19922 -16,-1.19922c-6.59766,0 -12.79687,0.5 -16.09766,1.19922c-2.19922,0.5 -4.10156,2 -4.5,4.30078c-0.40234,2.5 -0.80078,6 -0.80078,10.5c0,4.5 0.39844,8 0.89844,10.5c0.40234,2.19922 2.30078,3.80078 4.5,4.30078c3.5,0.69922 9.5,1.19922 16.10156,1.19922c6.60156,0 12.60156,-0.5 16.10156,-1.19922c2.19922,-0.5 4.09766,-2 4.5,-4.30078c0.39844,-2.5 0.89844,-6.10156 1,-10.5c-0.20312,-4.5 -0.70312,-8 -1.20312,-10.5zM19,32v-14l12.19922,7z"></path>
                                    </g>
                                </g>
                            </svg>
                        </a>
                    </div>
                    <div class="col-2">
                        <a href="https://www.tiktok.com/">
                            <!-- ICONO TIKTOK -->
                            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30px" viewBox="0,0,256,256">
                                <g fill="#ffffff" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt" stroke-linejoin="miter" stroke-miterlimit="10" stroke-dasharray="" stroke-dashoffset="0" font-family="none" font-weight="none" font-size="none" text-anchor="none" style="mix-blend-mode: normal">
                                    <g transform="scale(5.12,5.12)">
                                        <path d="M9,4c-2.75042,0 -5,2.24958 -5,5v32c0,2.75042 2.24958,5 5,5h32c2.75042,0 5,-2.24958 5,-5v-32c0,-2.75042 -2.24958,-5 -5,-5zM9,6h32c1.67158,0 3,1.32842 3,3v32c0,1.67158 -1.32842,3 -3,3h-32c-1.67158,0 -3,-1.32842 -3,-3v-32c0,-1.67158 1.32842,-3 3,-3zM26.04297,10c-0.5515,0.00005 -0.99887,0.44655 -1,0.99805c0,0 -0.01098,4.87522 -0.02148,9.76172c-0.0053,2.44325 -0.01168,4.88902 -0.01562,6.73047c-0.00394,1.84145 -0.00586,3.0066 -0.00586,3.10352c0,1.81526 -1.64858,3.29883 -3.52734,3.29883c-1.86379,0 -3.35156,-1.48972 -3.35156,-3.35352c0,-1.86379 1.48777,-3.35156 3.35156,-3.35156c0.06314,0 0.1904,0.02075 0.4082,0.04688c0.28415,0.03406 0.56927,-0.05523 0.78323,-0.24529c0.21396,-0.19006 0.33624,-0.46267 0.33591,-0.74885v-4.20117c-0.00005,-0.528 -0.41054,-0.965 -0.9375,-0.99805c-0.15583,-0.0098 -0.35192,-0.0293 -0.58984,-0.0293c-5.24953,0 -9.52734,4.27782 -9.52734,9.52734c0,5.24953 4.27782,9.52734 9.52734,9.52734c5.24938,0 9.52734,-4.27782 9.52734,-9.52734v-9.04883c1.45461,1.16341 3.26752,1.90039 5.26953,1.90039c0.27306,0 0.53277,-0.01618 0.78125,-0.03906c0.51463,-0.04749 0.90832,-0.47927 0.9082,-0.99609v-4.66992c0.0003,-0.52448 -0.40463,-0.9601 -0.92773,-0.99805c-3.14464,-0.22561 -5.65141,-2.67528 -5.97852,-5.79102c-0.05305,-0.50925 -0.48214,-0.89619 -0.99414,-0.89648zM27.04102,12h2.28125c0.72678,3.2987 3.30447,5.8144 6.63672,6.44531v2.86523c-2.13887,-0.10861 -4.01749,-1.1756 -5.12305,-2.85742c-0.24284,-0.36962 -0.69961,-0.53585 -1.12322,-0.40877c-0.4236,0.12708 -0.71344,0.51729 -0.71272,0.95955v11.53516c0,4.16848 -3.35873,7.52734 -7.52734,7.52734c-4.16848,0 -7.52734,-3.35887 -7.52734,-7.52734c0,-4.00052 3.12077,-7.17588 7.05469,-7.43164v2.17578c-2.71358,0.25252 -4.87891,2.47904 -4.87891,5.25586c0,2.94421 2.40735,5.35352 5.35156,5.35352c2.92924,0 5.52734,-2.30609 5.52734,-5.29883c0,0.04892 0.00186,-1.25818 0.00586,-3.09961c0.0039,-1.84143 0.01037,-4.28722 0.01563,-6.73047c0.0094,-4.3869 0.0177,-7.91447 0.01953,-8.76367z"></path>
                                    </g>
                                </g>
                            </svg>
                        </a>
                    </div>
                </div>
            </div>
            <div id="inferiorSeccionDerechaFooter" class="row">
                <a href="#top" class="text-center"><img src="media/images/logos/Logo_LA_BUENIZZIMA-1_SF.webp" width="50%" alt="Imagen La Buenizzima logo."></a>
            </div>
        </div>
    </div>
</footer>